...
def get_knn_top_m_songs(userid, k, m, tree, user_playlist_df, user_MUSIC_df):
    ...
    for i in range(len(closest_userids)):
        closest_user_songs.append(user_playlist_df.loc[user_playlist_df['user_id'] == closest_userids[i]]['playlist'].tolist()[0])
...
if __name__ == "__main__":
    ...
    get_knn_top_m_songs(sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), tree, user_playlist_df, user_MUSIC_df)