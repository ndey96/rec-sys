...
Start an EC2 Instance (took 40 minutes on a m5.2xlarge instance) with the Ubuntu 18.04 AMI in us-east-1a with the MSD EBS Volume attached to it. The links below should be helpful.
...
Then start a new shell session on your local machine and run `scp ubuntu@ec2-123.compute-1.amazonaws.com:rec-sys/msd_processing/full_msd.csv /some/local/directory` to grab the processed csv file.